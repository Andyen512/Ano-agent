...
GENDATA_RULE_PATH = SCRIPT_DIR / "rule_gendata_split.md"
GENDATA_V2_RULE_PATH = SCRIPT_DIR / "rule_gendata_v2.md"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--gendata-mode",
        action="store_true",
        help="Use generated data evaluation mode with rule_gendata_split.md",
    )
    parser.add_argument(
        "--gendata-v2-mode",
        action="store_true",
        help="Use same prompt for all models with rule_gendata_v2.md",
    )
    parser.add_argument(
        "--gendata-rule",
        type=Path,
        help="Custom rule file for gendata mode (default: rule_gendata_split.md)",
    )
    return parser
...
def valid_review_summary_gendata_v2(path: Path, expected_video_path: Path) -> bool:
    """Validate review summary for gendata v2 mode (1 agent instead of 5)."""
    if not path.exists():
        return False
    try:
...
def build_tasks(args: argparse.Namespace, data_root: Path, output_root: Path) -> tuple[list[VideoTask], list[str]]:
    ...
    for index, video_path in enumerate(videos, start=1):
        # Skip normal videos in gendata mode
        if args.gendata_mode or args.gendata_v2_mode:
            if "/normal/" in str(video_path) or "\\normal\\" in str(video_path):
                skipped_normal.append(str(video_path))
                continue
        review_root = task_output_root(output_root, data_root, video_path)
        summary_path = review_root / "review_summary.json"
        if args.resume:
            # Use different validation for gendata v2 mode
            if args.gendata_v2_mode:
                if valid_review_summary_gendata_v2(summary_path, video_path):
                    skipped.append(str(video_path))
                    continue
            elif valid_review_summary(summary_path, video_path):
                skipped.append(str(video_path))
                continue
        tasks.append(
            VideoTask(
    ...
    if skipped_normal:
        print(f"Skipped {len(skipped_normal)} normal videos", file=sys.stderr)
...
def load_prompts_file(prompts_path: Path) -> dict[str, str]:
    ...
    File format: video_id\tpath\ttype\tprompt_text\tsolutions
    """
    prompts = {}
...
def parse_gendata_v2_response(response: str) -> dict[str, Any]:
    """Parse gendata v2 format response."""
    result = {}
    for line in response.split("\n"):
        ...
        if ":" in line:
            ...
            if key == "scene_match":
                try:
                    result["scene_match"] = int(value)
                except ValueError:
                    pass
            elif key == "subject_match":
                try:
                    result["subject_match"] = int(value)
                except ValueError:
                    pass
            elif key == "risk_source_match":
                try:
                    result["risk_source_match"] = int(value)
                except ValueError:
                    pass
            elif key == "event_type_match":
                try:
                    result["event_type_match"] = int(value)
                except ValueError:
                    pass
            elif key == "video_type_observation":
                try:
                    result["video_type_observation"] = int(value)
                except ValueError:
                    pass
            elif key == "semantic_score":
                try:
                    result["semantic_score"] = float(value)
                except ValueError:
                    pass
            elif key == "total_score":
                try:
                    result["total_score"] = float(value)
                except ValueError:
                    pass
            elif key == "decision":
...
def is_valid_gendata_v2_response(response: str) -> bool:
    """Check if response is valid gendata v2 format."""
    parsed = parse_gendata_v2_response(response)
    required_fields = ["scene_match", "subject_match", "risk_source_match", "event_type_match", "semantic_score", "decision"]
    return all(field in parsed for field in required_fields)


def build_prompt_gendata(rule: PerspectiveRule, candidate_text: str | None) -> str:
    """Build prompt for generated video evaluation mode."""
    task_lines = [
        "You are evaluating a generated video for prompt-video consistency.",
        "Base your judgment only on the video content. Do not assume facts outside the video.",
    ]
    if candidate_text:
        task_lines.append(f"The original prompt for this video is: {candidate_text.strip()}")
    ...
    task_lines.append("scene_match: 0/1/2")
    task_lines.append("subject_match: 0/1/2")
    task_lines.append("risk_source_match: 0/1/2")
    task_lines.append("event_type_match: 0/1/2")
    task_lines.append("video_type_observation: 0/1/2")
    task_lines.append("semantic_description_score: [0, 1]")
    task_lines.append("prompt_video_consistency_score: [0, 10]")
    task_lines.append("final_decision: accept / weak_accept / reject")
    task_lines.append("explanation: brief explanation of your judgment")
...
def worker_main(
    ...
    try:
        ...
        gendata_v2_mode = args_payload.get("gendata_v2_mode", False)
        ...
        while True:
            ...
            try:
                ...
                # In v2 mode, check for valid gendata v2 response format
                if gendata_v2_mode:
                    if not is_valid_gendata_v2_response(response):
...
def write_video_summary(
    ...
    # Use different summarization based on mode
    if args.gendata_v2_mode:
        majority_vote = summarize_results_average(state.agent_results)
    else:
...
def summarize_results_average(agent_results: list[dict[str, Any]]) -> dict[str, Any]:
    """Summarize results by averaging scores across all models (for gendata v2 mode)."""
    ...
    if failed:
        return {
            "complete": False,
            "error": f"{len(failed)} agent(s) failed",
            "failed_agents": [item.get("model", {}).get("backend", "unknown") for item in failed],
        }
    
    if not successful:
        return {"complete": False, "error": "No successful results"}
    
    # Extract scores from responses using parse_gendata_v2_response
    scores = {
        "scene_match": [],
        "subject_match": [],
        "risk_source_match": [],
        "event_type_match": [],
        "video_type_observation": [],
        "semantic_score": [],
        "total_score": [],
    }
    decisions = []
    ...
    for key, values in scores.items():
        if values:
            ...
            avg_scores[f"avg_{key}"] = None
    
    # Determine final decision based on averages
    avg_total = avg_scores.get("avg_total_score", 0)
    avg_event = avg_scores.get("avg_event_type_match", 0)
    avg_semantic = avg_scores.get("avg_semantic_score", 0)
    
    if avg_total is not None and avg_event is not None and avg_semantic is not None:
        if avg_total >= 8 and avg_event >= 1 and avg_semantic >= 0.7:
    ...
    return {
        ...
        "expected_votes": len(agent_results),
    }
...
def main() -> int:
    ...
    if args.gendata_v2_mode:
        rule_path = GENDATA_V2_RULE_PATH
        if not rule_path.exists():
            raise FileNotFoundError(f"Gendata v2 rule file does not exist: {rule_path}")
    elif args.gendata_mode:
        rule_path = args.gendata_rule if args.gendata_rule else GENDATA_RULE_PATH
        if not rule_path.exists():
            raise FileNotFoundError(f"Gendata rule file does not exist: {rule_path}")
    else:
        rule_path = RULE_PATH
        if not rule_path.exists():
    ...
    # Load prompts file for gendata mode
    prompts_map = {}
    if args.gendata_mode or args.gendata_v2_mode:
        prompts_path = PROJECT_ROOT / "data" / "prompts_0529.txt"
        prompts_map = load_prompts_file(prompts_path)
        print(f"Loaded {len(prompts_map)} prompts from {prompts_path}", file=sys.stderr)
    
    if args.gendata_v2_mode:
        # In v2 mode, all models use the same prompt
        rule_text = rule_path.read_text(encoding="utf-8").strip()
        # Create a single rule for all models
        rules = [PerspectiveRule(index=1, name="gendata_v2", prompt=rule_text)]
        # Only use models that are in the worker plan
        available_backends_set = set(worker_backends)
        assignments = []
        for model in model_pool:
            if model.backend in available_backends_set:
                assignments.append((rules[0], model))
    else:
    ...
    manifest = {
        ...
        "gendata_mode": bool(args.gendata_mode),
        "gendata_v2_mode": bool(args.gendata_v2_mode),
        "rule_path": str(rule_path),
        "model_pool": [
            {
                "model_id": item.model_id,
    ...
    failed_log_path = output_root / "failed_videos.jsonl"
    ...
    def schedule_state(key: str, state: VideoState) -> None:
        ...
        for assignment_index in next_assignment_indices(args, state):
            ...
            # 从 prompts_map 中获取原始 prompt
            # 视频文件名格式: R000xxx_type.mp4
            candidate_text = args.candidate_text
            if not candidate_text and (args.gendata_mode or args.gendata_v2_mode):
                # 从文件名提取 video_id (例如: R000001_normal)
                video_stem = state.task.video_path.stem  # R000001_normal
                if video_stem in prompts_map:
                    candidate_text = prompts_map[video_stem]
                else:
                    # 回退到从目录名提取
                    parts = state.task.relative_path.parts
                    if len(parts) >= 4:
                        candidate_text = parts[3]
            
            # Use different prompt builder based on mode
            if args.gendata_v2_mode:
                # In v2 mode, use rule text directly as prompt
                prompt = rule.prompt
                if candidate_text:
                    prompt = f"{prompt}\n\nThe original prompt for this video is: {candidate_text}"
            elif args.gendata_mode:
                prompt = build_prompt_gendata(rule, candidate_text)
            else:
                prompt = build_prompt(rule, candidate_text)
    ...
    try:
        ...
        while active:
            ...
            if message["returncode"] != 0:
                state.failed = True
                failed_info = {
                    "video_key": str(state.task.relative_path),
                    "video_path": str(state.task.video_path),
                    "worker_id": message.get("worker_id", ""),
                ...
                failed_videos.append(failed_info)
                # 写入失败日志
                with open(failed_log_path, "a", encoding="utf-8") as f:
                    f.write(json.dumps(failed_info, ensure_ascii=False) + "\n")
    ...
    if failed_videos:
        print(f"\nFailed videos: {len(failed_videos)}", file=sys.stderr)
        print(f"Failed log: {failed_log_path}", file=sys.stderr)
    return 0
