...
BATCH_OUTPUT_STEM = "batch_review_summary.minicpmv45_all_conservative"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Replace every remaining Tarsier2-backed Conservative Safety Perspective "
            "with MiniCPM-V-4.5 and write repaired summaries without overwriting originals."
        )
    )
...
def effective_summary_path(summary_path: Path) -> Path:
    repaired_path = summary_path.with_name("review_summary.repaired.json")
    return repaired_path if repaired_path.exists() else summary_path


def is_tarsier2_agent(agent: dict[str, Any]) -> bool:
    model = agent.get("model", {})
    return str(model.get("backend", "")).strip().lower() == "tarsier2"
...
def discover_tasks(args: argparse.Namespace) -> list[RepairTask]:
    ...
    for summary_path in summary_paths:
        source_summary_path = effective_summary_path(summary_path)
        payload = read_json(source_summary_path)
        agent_pair = find_target_agent(payload)
        if agent_pair is None:
            continue
        assignment_index, original_agent = agent_pair
        if not is_tarsier2_agent(original_agent):
            continue
...
def write_batch_outputs(output_root: Path, tasks: list[RepairTask], rows: list[dict[str, Any]]) -> None:
    ...
    write_json(output_root / f"{BATCH_OUTPUT_STEM}.json", payload)
    write_csv(output_root / f"{BATCH_OUTPUT_STEM}.csv", rows)
...
def main() -> int:
    ...
    worker_logs = output_root / "worker_logs_minicpmv45_replace_remaining"
    ...
    summary = {
        ...
        "batch_json": str(output_root / f"{BATCH_OUTPUT_STEM}.json"),
        "batch_csv": str(output_root / f"{BATCH_OUTPUT_STEM}.csv"),
    }
