...
EVAL_OUTPUT_DIR="${LIFEBENCH_EVAL_OUTPUT_DIR:-${PROJECT_ROOT}/outputs/evaluation/real_videos/qwen3_5_9b}"
QWEN_PYTHON="${LIFEBENCH_QWEN_PYTHON:-/home/caiqingyuan/data/caiqingyuan/env/lifebench-qwen35/bin/python}"
...
# Eight simultaneous video decoders can otherwise exhaust the host thread
# limit even when GPU memory is available.
...
echo "[1/3] Starting Qwen3.5 inference on ${NUM_GPUS} GPUs"
for ((gpu = 0; gpu < NUM_GPUS; gpu++)); do
    log_file="${LOG_DIR}/qwen35_chunk${gpu}.log"
    ...
    PYTHONPATH="${PROJECT_ROOT}/.vendor/qwen35_shim${PYTHONPATH:+:${PYTHONPATH}}" \
    "$QWEN_PYTHON" "${SCRIPT_DIR}/benchmark_auto_infer.py" \
        --device "$gpu" \
        --model-keys qwen3.5-9b \
...
for ((gpu = 0; gpu < NUM_GPUS; gpu++)); do
    if wait "${INFER_PIDS[$gpu]}"; then
        ...
        echo "[infer] GPU ${gpu} failed; check ${LOG_DIR}/qwen35_chunk${gpu}.log" >&2
        INFER_FAILED=1
    fi
done
...
echo "[3/3] Evaluating with 8 Qwen3-8B endpoints"
